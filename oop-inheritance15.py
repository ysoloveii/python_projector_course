...
def test_open_account():
    ...
    account = bank.open_account('SavingsAccount', 1000, 'SA0001', 0.05)

    # check that the account was opened and has the correct balance and interest rate
    assert isinstance(account, SavingsAccount)
    assert account.get_balance() == 1000
    assert account.get_interest_rate() == 0.05

    account = bank.open_account('CurrentAccount', 500, 'CA0001', 100)

    # check that the account was opened and has the correct balance and overdraft limit
    assert isinstance(account, CurrentAccount)
    assert account.get_balance() == 500
    assert account.get_overdraft_limit() == 100


def test_update(capsys):
    """Test update method. 
    Check that code added interest and sended a message."""
    bank = Bank()

    # check has had interest added after calling the update method
    account1 = bank.open_account('SavingsAccount', 1000, 'SA0001', 0.05)

    # check has had an overdraft letter sent after calling the update method
    account2 = bank.open_account('CurrentAccount', -500, 'CA0001', 100)
    bank.update()

    # check that the interest has been added correctly
    assert account1.get_balance() == 1050

    # check that the CurrentAccount has had a message sent
    captured = capsys.readouterr()
    assert "Sending overdraft letter" in captured.out
